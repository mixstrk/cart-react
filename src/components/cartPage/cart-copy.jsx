...
import AppMinMax from "./minmax/minmax";
  ...
  const proceedToCheckoutOrder = () => {
    navigate("/order", {
      state: {
        ...
        totalProductsCount: getTotalProductsCount(products),
      },
    });
  };
  ...
  const cartPage = (
    <div>
      ...
      <div style={{ textAlign: "right" }}>
        <span>
          Total({getTotalProductsCount(products)}):{" "}
          <strong>{getTotalPrice(products)}₽</strong>
        </span>
        <div>
...
function showCongrats(totalPrice) {
  return (
    <div>
      ...
      <span>
        The order cost is <strong>{totalPrice}₽</strong>
      </span>
    </div>
  );
}
